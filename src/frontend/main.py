...
def initialize_openai_client(base_url: str, api_key: str = "not-needed") -> openai.OpenAI:
...
def generate_speech(client: openai.OpenAI, text: str, voice: str = "expr-voice-2-f") -> bytes:
...
with st.sidebar:
    ...
    if st.button("Connect to API") or st.session_state.client is None:
        try:
            ...
            st.session_state.available_models = get_available_models(st.session_state.client)
